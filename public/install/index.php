...
define("VERSION", 'ThinkPHP 5.1');

if(is_file(SITEDIR."public".DIRECTORY_SEPARATOR.'install.lock')){
    exit("HuiCMF程序已运行安装，如果你确定要重新安装，请先从FTP中删除 public/install.lock！");
}

@set_time_limit(1000);

if (version_compare(PHP_VERSION,'7.0','<')) exit('您的php版本过低，不能安装本软件，请升级到7.0或更高版本再安装，谢谢！');
...
switch ($step) {
    ...
    case '2':
        if (phpversion() < 7) {
            die('本系统需要PHP7.0+MYSQL >=5.0环境，当前PHP版本为：' . phpversion());
        }
        ...
        if(in_array('mbstring', get_loaded_extensions ())){
            $mbstring = '<span class="correct_span">&radic;</span> 支持';
        }else{
    ...
    case '4':
        if (isset($_GET['install'])) {
            ...
            $config['type'] = in_array($dbType, array('mysql')) ? $dbType : 'mysql';
            $config['hostname'] = $dbHost;
            $config['database'] = $dbName;
            $config['username'] = $dbUser;
            $config['password'] = $dbPwd;
            $config['hostport'] = $dbPort;
    ...
    case '5':
        ...
        @touch(SITEDIR."public".DIRECTORY_SEPARATOR.'install.lock');
        @unlink(SITEDIR."public".DIRECTORY_SEPARATOR.'install.php');
        exit;
}

function testwrite($d) {
    $tfile = "huicmf_test.txt";
    $fp = @fopen($d . "/" . $tfile, "w");
    if (!$fp) {
        return false;
    }
...
function set_config($config) {
    $configfile = SITEDIR.DIRECTORY_SEPARATOR.'config/database.php';
