...
class Index extends Common
{
    ...
    public function public_welcome()
    {
        ...
        $getinfo['last_logintime'] = ! empty(session('last_logintime')) ? date("Y-m-d H:i:s",
            session('last_logintime')) : '第一次登录';

        return $this->fetch('welcome', [
            'getinfo'    => $getinfo,
            'sqlversion' => $rs[0]['sqlversion']
        ]);
    }
    ...
    public function public_showmenu()
    {
        if (Cache::get('index_menu_list'.Session::get('roleid'))) {
            ...
            $menu_list = Db::name('menu')->field('`id`,`name`,`m`,`c`,`a`,`data`')->where([
                    'parentid' => 0,
                    'display'  => 1
            ...
            foreach ($menu_list as $key => $value) {
                $child = Db::name('menu')->field('`id`,`name`,`m`,`c`,`a`,`data`')->where([
                        'parentid' => $value['id'],
                        'display'  => 1
                    ])->order('listorder ASC')->select();
                ...
                foreach ($child as $k => $v) {
                    if (Session::get('roleid') != 1) {
                        $data = Db::name('admin_role_priv')->field('roleid')->where([
